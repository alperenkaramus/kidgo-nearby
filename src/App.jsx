...
const COPY = {
  tr: {
    ...
    bestNow: 'Bugünün seçimi', cityPick: 'Şehir önerisi', score: 'uygunluk', demandTitle: 'Popüler aile planları', allCitiesTitle: 'Nereye gidiyorsunuz?', allCitiesHelp: 'Önce ülke ve şehir seç, sonra yaş ve ruh haline göre listeyi daralt. Başlangıçta popüler şehir önerileri ve güvenli yedek liste kullanılır.', citiesDefault: 'Şehir seç',
    ...
    noticeGuide: 'Güncel Wikipedia/Wikivoyage şehir kaynakları ve açık harita verileri birleştirildi; puan ve saatleri Google bağlantısından son kez doğrula.',
    ...
    noticeGeoErr: 'Konum alınamadı; şehir yazarak devam edebilirsin.', emptyTitle: 'Net eşleşme yok', emptyBody: 'Tümü seç veya başka bir şehir dene.', loadingTitle: 'Aile planı hazırlanıyor…', loadingBody: 'Şehir, yaş, mod ve kategori sinyallerine göre en uygun duraklar sıralanıyor.', map: 'Haritada aç', directions: 'Yol tarifi al', google: 'Google’da aç', rainy: 'Kapalı alan', whyPick: 'Neden uygun', confidence: 'Güven', scoreMix: 'Uygunluk bileşimi', liveGoogle: 'Canlı Google', openNow: 'Şu an açık', showResults: 'öneriyi göster', practical: 'Pratik notlar', ratingPending: 'Kullanıcı puanı gösterilmiyor', reviews: 'yorum', points: 'uygunluk', checkMap: 'Konumu haritada kontrol et', currentLocationLabel: 'Mevcut konum', confidenceHigh: 'yüksek', confidenceMedium: 'orta', confidenceStarter: 'başlangıç',
  },
  en: {
    ...
    bestNow: 'Today’s pick', cityPick: 'City pick', score: 'fit', demandTitle: 'Popular family plans', allCitiesTitle: 'Where are you going?', allCitiesHelp: 'Choose a country and city, then tune the list by age and mood. Launch mode uses curated popular-city data plus safe fallback.', citiesDefault: 'Pick a city', filtersTitle: 'Kid profile', intentTitle: 'Today’s mood', radius: 'Search radius', resultsTitle: 'Family picks ranked for today', searching: 'Planning…', ideas: 'ideas',
    noticeLive: 'Live OpenStreetMap results ranked by kid fit, distance and family-friendly signals.', noticeGuide: 'Current Wikipedia/Wikivoyage city references are combined with open-map data; verify ratings and hours through Google before going.', noticeFallback: 'Showing prepared family-safe city picks. Open the map link for the final local check.', noticeGeoNo: 'Browser geolocation is unavailable; city search stays active.', noticeGeoUse: 'Waiting for location permission… nearby picks will load if access is allowed.', noticeGeoReady: 'Location received. Preparing named nearby places and city backups.', noticeGeoLowAccuracy: 'Browser location is not precise enough. Turn on Precise Location or choose a city for better results.', noticeGeoErr: 'Location was unavailable; type a city to continue.', emptyTitle: 'No exact matches yet', emptyBody: 'Try All categories or another global city.', loadingTitle: 'Preparing a family plan…', loadingBody: 'Ranking places by city, age, mood and category fit.', map: 'Open map', directions: 'Get directions', google: 'Open in Google', rainy: 'Rainy day', whyPick: 'Why it fits', confidence: 'Confidence', scoreMix: 'Fit breakdown', liveGoogle: 'Live Google', openNow: 'Open now', showResults: 'show picks', practical: 'Practical notes', ratingPending: 'User rating not shown', reviews: 'reviews', points: 'fit', checkMap: 'Check location on map', currentLocationLabel: 'Current location', confidenceHigh: 'high', confidenceMedium: 'medium', confidenceStarter: 'starter',
  },
  ru: {
...
function App() {
  ...
  const evidenceCopy = {
    ...
    'mentioned in a current city guide': lang === 'tr' ? 'güncel şehir rehberinde adı geçiyor' : (lang === 'ru' ? 'упоминается в актуальном городском путеводителе' : (lang === 'de' ? 'in einem aktuellen Stadtführer erwähnt' : 'mentioned in a current city guide')),
  };
  ...
  useEffect(() => {
    ...
    async function runSearch() {
      ...
      try {
        ...
        else if (coords) setNotice(t.noticeGeoReady);
        else if (results.some((place) => place.guideMention)) setNotice(t.noticeGuide);
        else setNotice(results.some((place) => place.source?.toLowerCase().includes('fallback') || place.source?.toLowerCase().includes('seed')) ? t.noticeFallback : t.noticeLive);
      } catch (error) {
        if (cancelled) return;
        setStatus('error');
  ...
  }, [age, intent, category, coords, radiusKm, debouncedLocation, geoNoticeOverride, t.noticeFallback, t.noticeGuide, t.noticeLive, t.noticeGeoReady, t.noticeGeoErr]);
          ...
          return (
            <article className="place-card" key={place.id}>
              ...
              <div className="place-body">
                <div className="card-topline"><span className="category-tag">{cat?.labels[lang] || place.categoryLabel}</span>{place.guideMention && <a className="guide-tag" href={place.guideUrl} target="_blank" rel="noreferrer">{place.guideSource || 'City guide'} ✓</a>}{place.googleLive && <span className="google-live-tag">{t.liveGoogle}</span>}{place.openNow && <span className="open-tag">{t.openNow}</span>}{place.rainyDay && <span className="rain-tag">{t.rainy}</span>}</div>
                <h3>{place.name}</h3>
